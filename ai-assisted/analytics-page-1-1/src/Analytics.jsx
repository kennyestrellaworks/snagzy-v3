...
function StatCard({ icon: Icon, label, value, sub, color, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`bg-white rounded-2xl p-6 shadow-sm border flex items-start gap-4 w-full text-left transition-all duration-200 ${active ? "border-blue-500 ring-2 ring-blue-500/20" : "border-gray-100 hover:border-gray-200 hover:shadow-md cursor-pointer"}`}
    >
      <div
...
export default function Analytics() {
  ...
  const stats = useMemo(() => {
    const totalRevenue = orders.reduce(
      (s, o) => s + o.summary.orderTotalPrice,
      0,
    );
    ...
    const totalItems = orders.reduce(
      (s, o) =>
        s + o.orderedItems.reduce((ss, i) => ss + i.variant.quantity, 0),
      0,
    );
    const avgOrderValue = totalRevenue / totalOrders;

    const successfulOrders = orders.filter((o) =>
      successfulOrderStatuses.includes(o.currentStatus.slug),
    );
    ...
    const pendingOrders = orders.filter((o) =>
      pendingOrderStatuses.includes(o.currentStatus.slug),
    );
    ...
    const cancelledOrders = orders.filter((o) =>
      unsuccessfulOrderStatuses.includes(o.currentStatus.slug),
    );
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (selectedFilter === "totalSales") return stats.successfulOrders;
    if (selectedFilter === "pendingSales") return stats.pendingOrders;
    if (selectedFilter === "cancellations") return stats.cancelledOrders;
    return [];
  }, [
    selectedFilter,
      ...
      o.orderedItems.forEach((item) => {
        ...
        if (!map[id])
          map[id] = {
            ...
            qty: 0,
            revenue: 0,
            image: item.variant.primaryImage,
          };
  ...
  const revenueData =
    revenueView === "monthly" ? revenueByMonth : revenueByMonth;
  ...
  const orderLifecycleProgress = useMemo(() => {
    ...
    const stageData = [];
    const sequencedStages = orderLifeCycle
      .filter((s) => s.sequence <= 9)
      ...
      const avgTimeInStage =
        ordersAtStage > 0
          ? orders
              ...
              .reduce((sum, o) => {
                const thisStageIdx = o.timeline.findIndex(
                  (t) => t.slug === stage.slug,
                );
                ...
                const thisTime = new Date(
                  o.timeline[thisStageIdx].timestamp,
                ).getTime();
                const nextTime = new Date(
                  o.timeline[nextStageIdx].timestamp,
                ).getTime();
        ...
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Order performance and revenue insights
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-3 gap-4">
          <StatCard
            ...
            color="bg-green-600"
            active={selectedFilter === "totalSales"}
            onClick={() =>
              setSelectedFilter(
                selectedFilter === "totalSales" ? null : "totalSales",
              )
            }
          />
          <StatCard
            ...
            color="bg-amber-500"
            active={selectedFilter === "pendingSales"}
            onClick={() =>
              setSelectedFilter(
                selectedFilter === "pendingSales" ? null : "pendingSales",
              )
            }
          />
          <StatCard
            ...
            color="bg-red-500"
            active={selectedFilter === "cancellations"}
            onClick={() =>
              setSelectedFilter(
                selectedFilter === "cancellations" ? null : "cancellations",
              )
            }
          />
        </div>

        {selectedFilter && filteredOrders.length > 0 && (
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              <SectionHeader
                title={
                  selectedFilter === "totalSales"
                    ? "Completed Orders"
                    : selectedFilter === "pendingSales"
                      ? "Pending Orders"
                      : "Cancelled Orders"
                }
                      ...
                      >
                        ...
                        <td className="py-3 pl-4">
                          <span
                            ...
                            style={{
                              backgroundColor: `${STATUS_COLORS[order.currentStatus.slug] || "#94a3b8"}15`,
                              color:
                                STATUS_COLORS[order.currentStatus.slug] ||
                                "#64748b",
                            }}
                          >
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <SectionHeader
            ...
            subtitle="Orders progressing through stages with average time per stage"
          />
          <ResponsiveContainer width="100%" height={320}>
            <BarChart
              data={orderLifecycleProgress}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              >
                ...
                >
                  {orderStatusDist.map((entry, index) => (
                    <Cell
                      key={index}
                      fill={STATUS_COLORS[entry.slug] || "#94a3b8"}
                    />
                  ))}
                </Bar>
              </BarChart>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <SectionHeader
              ...
              subtitle="Breakdown by payment provider"
            />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <SectionHeader
              ...
              subtitle="Distribution by delivery method"
            />
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <SectionHeader
              title="Carriers"
              subtitle="Orders fulfilled per carrier"
            />
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <SectionHeader
            ...
            subtitle="Best-performing products across all orders"
          />
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-right text-xs font-semibold text-gray-400 uppercase tracking-wide pb-3 px-4">
                    Qty Sold
                  </th>
                  <th className="text-right text-xs font-semibold text-gray-400 uppercase tracking-wide pb-3">
                    Revenue
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <SectionHeader
            ...
            subtitle="Highest-grossing stores across all orders"
          />
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
