...
import { Globe, MapPin, Building, Shuffle, Copy, Check, Pencil, Navigation } from 'lucide-react';

const streetNames = [
  'Main', 'Oak', 'Maple', 'Cedar', 'Pine', 'Elm', 'Washington',
  'Lincoln', 'Park', 'Lake', 'Hill', 'River', 'Forest', 'Sunset',
  'Highland', 'Spring', 'Valley', 'Bay', 'Harbor', 'Meadow',
];

const streetTypes = [
  'Street', 'Avenue', 'Boulevard', 'Drive', 'Lane', 'Court',
  'Place', 'Way', 'Circle', 'Road', 'Terrace', 'Trail',
];
...
function generateAddressLine1() {
  ...
  const directional = Math.random() > 0.5 ? ` ${randomFrom(['N', 'S', 'E', 'W'])}` : '';
...
export default function AddressGenerator() {
  ...
  const handleGenerate = useCallback(() => {
    ...
    const lat = selectedCityData?.latitude || selectedState?.latitude || selectedCountry?.latitude;
    const lng = selectedCityData?.longitude || selectedState?.longitude || selectedCountry?.longitude;
    if (lat && lng) {
    ...
    setCopied(false);
  }, [countryCode, stateCode, resolvedCity, selectedState, selectedCountry, selectedCityData]);
    ...
    <div className="w-full max-w-xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/60 border border-slate-200/80 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-slate-800 to-slate-900 px-8 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold text-white tracking-tight">Address Generator</h1>
              <p className="text-sm text-slate-400 mt-0.5">Select a location, generate an address</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-8 pt-8 pb-4 space-y-5">
          ...
          <div>
            ...
            >
              <option value="">
                {countryCode ? 'Choose a state / region' : 'Select a country first'}
              </option>
              {states.map((s) => (
            ...
            {!stateCode ? (
              ...
              <div className="space-y-2">
                <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  No cities in database for this region. Please type the city name below.
                </p>
        ...
        {generatedAddress && (
          <div className="mx-8 mb-8 rounded-xl bg-slate-800 text-white overflow-hidden animate-fade-in-up">
            ...
            <div className="px-5 py-4 space-y-3">
              <div>
                <span className="text-xs font-medium uppercase tracking-wider text-slate-500">Address Line 1</span>
                <p className="text-sm font-mono mt-0.5">{addressLine1}</p>
              </div>
              {addressLine2 && (
                <div>
                  <span className="text-xs font-medium uppercase tracking-wider text-slate-500">Address Line 2</span>
                  <p className="text-sm font-mono mt-0.5">{addressLine2}</p>
                </div>
              )}
              <div>
                <span className="text-xs font-medium uppercase tracking-wider text-slate-500">City, State ZIP</span>
                <p className="text-sm font-mono mt-0.5">{generatedAddress.split('\n').filter(l => l !== addressLine1 && l !== addressLine2).join('\n')}</p>
              </div>
              {coordinates && (
                <div className="pt-2 border-t border-slate-700/50">
                  <span className="text-xs font-medium uppercase tracking-wider text-slate-500">Coordinates</span>
                  <div className="flex items-center gap-3 mt-1.5">
                    <Navigation className="w-4 h-4 text-emerald-400 shrink-0" />
                    <div className="text-sm font-mono">
                      <span className="text-emerald-400">{coordinates.lat.toFixed(4)}</span>
                      <span className="text-slate-600 mx-1">,</span>
                      <span className="text-emerald-400">{coordinates.lng.toFixed(4)}</span>
                    </div>
                    <span className="text-xs text-slate-600">
                      ({coordinates.lat >= 0 ? `${coordinates.lat.toFixed(4)} N` : `${(-coordinates.lat).toFixed(4)} S`}, {coordinates.lng >= 0 ? `${coordinates.lng.toFixed(4)} E` : `${(-coordinates.lng).toFixed(4)} W`})
                    </span>
                  </div>
                </div>
              )}
      ...
      {selectedCountry && (
        <div className="mt-4 flex items-center justify-center gap-2 text-sm text-slate-500">
          ...
          <span>
            {[selectedCountry?.flag, selectedState?.name, resolvedCity].filter(Boolean).join(' / ')}
          </span>
        </div>
      )}
    </div>
